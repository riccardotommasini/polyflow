...
public class NaryDAGNodeImpl<R extends Iterable<?>> implements DAGNode<R> {
    ...
    public NaryDAGNodeImpl(RelationToRelationOperator<R> r2rOperator) {
    ...
    public R eval(long ts) {
        return this.r2rOperator.eval(prev.stream().map(p -> p.eval(ts)).toList());
    }
